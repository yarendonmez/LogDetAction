...
results_path = Path(r"C:\developer\LogDetAction\v2.0\evaluation_qlora_classifier_results.csv")
...
def to_attack_label(label):
    """
    3-class etiketi attack / non_attack formatına çevirir.
    """
    if label == "malicious":
        return "attack"
...
def build_prompt(log_entry):
    """
    Eğitim formatıyla uyumlu classifier-only prompt.
...
print("\nClassifier-only QLoRA evaluation sonuçları kaydedildi:")
print(results_path)
